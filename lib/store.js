...
export const BASE_DIR = path.join(homedir(), ".matter-cli");
...
export function loadDevices() {
  try {
    ...
    throw new Error(`Failed to read ${DEVICES_FILE} — file may be corrupted`);
  }
}
...
export function saveWifi(ssid, password) {
  const wifi = loadWifi();
  wifi[ssid] = password;
  fs.writeFileSync(WIFI_FILE, JSON.stringify(wifi, null, 2), { mode: 0o600 });
}
